...
# 脚本职责边界：扩展统一词典配置，输出扩展后的字典文件。
# 主要功能：读取 `config/dictionaries.yaml`，生成 `config/dictionaries_expanded.yaml`。

REPO_ROOT = Path(__file__).resolve().parents[3]
CONFIG_PATH = REPO_ROOT / "config" / "dictionaries.yaml"
OUTPUT_PATH = REPO_ROOT / "config" / "dictionaries_expanded.yaml"
