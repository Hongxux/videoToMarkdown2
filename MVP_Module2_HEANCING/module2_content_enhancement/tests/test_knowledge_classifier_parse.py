...
def _load_knowledge_classifier_module():
    # 做什么：通过文件路径加载 knowledge_classifier.py，避免导入 package __init__ 触发 cv2 等重依赖。
    # 为什么：测试目标仅覆盖 _parse_batch_content，避免环境缺少 OpenCV 时无法收集测试。
    # 权衡：跳过包级导入路径，属于测试内的隔离手段。
    module_path = Path(__file__).resolve().parent.parent / "knowledge_classifier.py"
    spec = importlib.util.spec_from_file_location("module2_knowledge_classifier", module_path)
