...
class _TraceLLMClient:
    async def complete_text(self, prompt: str, system_message: str = None):
        ...
        if "教学文本补全助手" in sys_msg:
            return "补全文本", _Meta(), None
        if "教学内容结构化助手" in sys_msg:
            return "结构化正文", _Meta(), None
        # hierarchy
...
class _RecorderAugmentLLMClient:
    ...
    async def complete_text(self, prompt: str, system_message: str = None):
        ...
        if "教学文本补全助手" in system:
            return "先打开终端，然后执行 npm run dev。", None, None

        if "教学内容结构化助手" in system:
            if "先打开终端，然后执行 npm run dev。" in prompt:
                return "结构化包含命令 npm run dev", None, None
            return "结构化未包含命令", None, None
...
def test_concrete_section_embed_preserves_assets_subdirectory(tmp_path, monkeypatch):
    ...
    enhancer = MarkdownEnhancer()
    enhancer._enabled = True
    enhancer._llm_client = _FakeLLMClient("- key step\n銆恑mgneeded_SU888_img_01銆慭n- end")
...
def test_process_non_tutorial_renders_multiple_videos(tmp_path, monkeypatch):
    ...
    enhancer = MarkdownEnhancer()
    enhancer._enabled = True
    enhancer._llm_client = _FakeLLMClient("step details\n銆恑mgneeded_SU201_img_01銆慭ndone")
...
def test_concrete_section_uses_image_desc_augment_before_structuring_when_enabled(tmp_path, monkeypatch):
                    ...
                    "screenshot_items": [
                        {
                            "img_id": "SU300_img_01",
                            "img_path": str(img_path),
                            "img_description": "终端里显示命令 npm run dev",
                            "timestamp_sec": 5.2,
                            "sentence_id": "S002",
                            "sentence_text": "执行启动命令",
                        }
                    ],
                    "clip": "",
                    "action_classifications": [],
    ...
    assert "结构化包含命令 npm run dev" in markdown
    assert sum(1 for call in recorder.calls if "教学文本补全助手" in call["system_message"]) == 1
    assert sum(1 for call in recorder.calls if "教学内容结构化助手" in call["system_message"]) == 1
...
def test_concrete_section_skips_image_desc_augment_when_disabled(tmp_path, monkeypatch):
    ...
    assert "结构化未包含命令" in markdown
    assert sum(1 for call in recorder.calls if "教学文本补全助手" in call["system_message"]) == 0
    assert sum(1 for call in recorder.calls if "教学内容结构化助手" in call["system_message"]) == 1
