...
async def step7b_node(state: PipelineState) -> Dict[str, Any]:
    ...
    类型：LLM
    核心动作：识别文字稿中"适合可视化"的内容（独立于断层判断）
    
    优化：使用asyncio.gather并发处理所有segment（DeepSeek无速率限制）
    """
...
async def step7c_node(state: PipelineState) -> Dict[str, Any]:
    ...
    类型：LLM
    核心动作：识别同一知识点的不同DARPA维度，合并为knowledge_point
    """
