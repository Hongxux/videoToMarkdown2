...
async def step16_node(state: PipelineState) -> Dict[str, Any]:
    ...
    类型：LLM
    核心动作：判断知识点(Knowledge Point)是否需要可视化补充
...
async def step17_node(state: PipelineState) -> Dict[str, Any]:
    ...
    类型：代码规则
    核心动作：根据断层类型或可视化场景类型选择适配形式
    
    改造点：适配 KP 模式，聚合 KP 下所有 segment 的形式需求
    """
...
async def _refine_video_boundary(
    ...
    """
    [DEPRECATED] 使用 Vision API 精细化视频边界
    
    注意：此函数已被弃用，请使用 _verify_candidates_with_vision + _refine_with_retry
    """
    import warnings
...
async def _verify_candidates_with_vision(
    ...
    """
    使用 Vision AI 从完整帧序列中判断边界
    
    职责：接收 SSIM 采样的完整帧序列，根据场景标准返回最佳起止帧标签
    
    Args:
        all_frames: SSIM 采样的所有帧数据
        candidate_frames: 帧路径列表（由调用方准备）
        frame_labels: 帧标签列表（由调用方准备）
        title: 知识点标题
        summary: 知识点摘要
    
    Returns:
        {
            "boundary_status": "found" | "need_expand_start" | "need_expand_end" | "need_expand_both",
            "start_frame_label": str (仅当 status="found"),
            "end_frame_label": str (仅当 status="found"),
            "scene_type": str,
            "reason": str,
            "expand_suggestion": str
        }
    """
