...
async def step20_node(state: PipelineState) -> Dict[str, Any]:
    ...
    - 条件：not sid
    依据来源（证据链）：
    - 配置字段：post_media_summary, video_transition。
    输入参数：
...
async def step21_node(state: PipelineState) -> Dict[str, Any]:
    ...
    核心价值：封装逻辑单元，提升复用与可维护性。
    决策逻辑：
    - 条件：sid not in segment_faults
    - 条件：not full_text
    - 条件：form == 'none' and (not faults)
    依据来源（证据链）：
    输入参数：
    - state: 函数入参（类型：PipelineState）。
    输出参数：
    ...
    try:
        ...
        async def process_material(material):
            ...
            - 条件：not full_text
            - 条件：form == 'none' and (not faults)
            - 条件：not extracted or not any(extracted.values())
            依据来源（证据链）：
            输入参数：
            - material: 函数入参（类型：未标注）。
            输出参数：
            - 结构化字典结果（包含字段：segment_id, segment_ids, knowledge_point, structured_text, darpa_question, transition, _tokens）。"""
...
def _replace_media_placeholders(text: str, material: Dict) -> str:
    ...
    - 条件：not media_list
    - 条件：match_id
    - 条件：m_data and current_match
    依据来源（证据链）：
    输入参数：
    - text: 文本内容（类型：str）。
    - material: 函数入参（类型：Dict）。
    输出参数：
    - 字符串结果。"""
    import re
...
def _generate_media_embed_v2(media: Dict, parent_indent: str) -> str:
    ...
    决策逻辑：
    - 条件：parent_indent
    - 条件：media['type'] == 'video' or media['type'] == 'video_file'
    - 条件：media['type'] == 'video'
    依据来源（证据链）：
    - 输入参数：media, parent_indent。
    - 配置字段：type。
    输入参数：
    - media: 函数入参（类型：Dict）。
    - parent_indent: 函数入参（类型：str）。
    输出参数：
    - 字符串结果。"""
    lines = []
    aux = media.get("aux", {})
...
async def step22_node(state: PipelineState) -> Dict[str, Any]:
    ...
    核心价值：封装逻辑单元，提升复用与可维护性。
    决策逻辑：
    - 条件：reconstructed_materials
    - 条件：reconstructed_materials[0].get('darpa_question')
    - 条件：dq_list
    依据来源（证据链）：
    - 配置字段：darpa_question, knowledge_point。
    输入参数：
    - state: 函数入参（类型：PipelineState）。
    输出参数：
    - 结构化结果字典（包含关键字段信息）。"""
...
def _copy_resources(video_path: str, materials: List[Dict], notes_dir: Path):
    ...
    - 条件：video_path
    - 条件：src_video.exists()
    - 条件：not dst_video.exists()
    依据来源（证据链）：
    - 输入参数：video_path。
    输入参数：
    - video_path: 文件路径（类型：str）。
    - materials: 函数入参（类型：List[Dict]）。
    - notes_dir: 目录路径（类型：Path）。
    输出参数：
...
async def step22b_node(state: PipelineState) -> Dict[str, Any]:
    ...
    核心价值：封装逻辑单元，提升复用与可维护性。
    决策逻辑：
    - 条件：sid not in fault_map
    - 条件：processed_frames
    - 条件：sid not in screenshot_map
    依据来源（证据链）：
    - 配置字段：video_needed。
    输入参数：
    - state: 函数入参（类型：PipelineState）。
