...
class ResourceOrchestrator:
    """类说明：ResourceOrchestrator 负责封装本模块相关能力。
    执行步骤：
    ...
    def get_adaptive_concurrency(base_multiplier: float = 1.0, cap: int = 16) -> int:
        ...
        # 1. High-risk mode: low free RAM or memory usage > 90%
        ...
        # 2. Constrained mode: free RAM 2-6GB or usage 75-90%
        ...
        # 3. Normal mode: scale concurrency by CPU and free RAM
        # suggested = CPU * 1.5 * (free_gb / 8GB)
    ...
    def get_adaptive_cache_size(base_size: int = 50, per_gb_increment: int = 25) -> int:
        ...
        # Add 25 cache entries per extra 1GB, capped at 1000
        increment = int(available_gb * per_gb_increment)
        return min(base_size + increment, 1000)

class AdaptiveSemaphore:
    """类说明：AdaptiveSemaphore 负责封装本模块相关能力。
    执行步骤：
    1) 步骤1：接收调用请求并组织上下文数据。
    2) 步骤2：协调类内方法完成业务处理。
    3) 步骤3：输出处理结果并提供可复用能力。"""
    ...
    async def __aenter__(self):
        # Context manager entry: acquire semaphore before processing
        """
        执行逻辑：
        1) 准备必要上下文与参数。
        2) 执行核心处理并返回结果。
