...
from services.python_grpc.src.content_pipeline.shared.models.data_structures import (
    CorrectedSubtitle,
...
def sanitize_module_input(module_input: Module2Input) -> Module2Input:
    ...
    输出参数：
    - 函数计算/封装后的结果对象。"""
    duration = get_video_duration(module_input.video_path, default=0.0, use_cv2_fallback=True)
    if duration <= 0:
        return module_input
