    ...
    try:
        ...
        async def process_batch(idx, batch):
            ...
            try:
                ...
                return batch_corrected, batch_corrections, response.total_tokens, llm_latency_ms, None
                ...
                return fallback, [], 0, (time.perf_counter() - llm_started_at) * 1000, e
        ...
        success_batches = 0
        failed_batches = 0
        for batch_corrected, batch_corrections, tokens, latency_ms, error in results:
    ...
    try:
        ...
        async def process_window(idx, i):
            ...
            try:
                ...
                return enriched_sentences, response.total_tokens, (time.perf_counter() - llm_started_at) * 1000, None
            except Exception as e:
                return [], 0, (time.perf_counter() - llm_started_at) * 1000, e

        logger.info(f"并发执行 {len(corrected)} 条字幕的语义合并...")
        logger.log_substep("llm_window", "正在进行LLM窗口调用")
        ...
        for enriched_sentences, tokens, latency_ms, error in results:
            total_tokens += tokens
            total_latency_ms += latency_ms
            if error is None:
                success_windows += 1
    ...
    try:
        ...
        async def process_batch(idx, batch):
            ...
            try:
                ...
                return batch_results, response.total_tokens, (time.perf_counter() - llm_started_at) * 1000, None
            except Exception as e:
                return [], 0, (time.perf_counter() - llm_started_at) * 1000, e
        ...
        success_batches = 0
        failed_batches = 0
        for batch_results, tokens, latency_ms, error in results:
    ...
    try:
        ...
        async def process_window(idx, i):
            ...
            try:
                ...
                return (
                    result.get("keep_sentence_ids", []),
                    result.get("deleted_sentences", []),
        ...
        success_windows = 0
        failed_windows = 0
        for k_ids, d_info, tokens, latency_ms, error in results:
...
async def step6_node(state: PipelineState) -> Dict[str, Any]:
    ...
    try:
        ...
        success_windows = 0
        failed_windows = 0
        for window_paragraphs, tokens, latency_ms, error in results:
            total_tokens += tokens
            total_latency_ms += latency_ms
            if error is None:
                success_windows += 1
