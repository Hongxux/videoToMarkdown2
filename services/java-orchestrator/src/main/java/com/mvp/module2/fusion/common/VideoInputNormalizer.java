 ...
 * 目标：
 * 1. 兼容“标题 + 链接”混合文本；
 * 2. 兼容 https:\b23.tv\xxx 这类斜杠异常写法；
 * 3. 不破坏本地路径（含 Windows 绝对路径）。
 */
public final class VideoInputNormalizer {
    ...
    private VideoInputNormalizer() {
        // 工具类不需要实例化。
    }

    public static String normalizeVideoInput(String rawVideoInput) {
        ...
        if (bvMatcher.find()) {
            return "https://www.bilibili.com/video/" + keepBvIdOriginalCase(bvMatcher.group());
        }
        return trimmed;
    }

    public static boolean looksLikeLocalPath(String value) {
    ...
    private static String keepBvIdOriginalCase(String rawBvId) {
        if (rawBvId == null || rawBvId.isBlank()) {
            return "";
        }
